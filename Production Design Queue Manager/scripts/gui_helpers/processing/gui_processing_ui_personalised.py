"""Personalised GUI processing flow."""
...
def process_personalised_file(gui, df, order_column, sku_column, file_path=None, show_progress=True):
    """Process personalised designs for one file."""
    ...
    try:
        ...
        log_stats = {'total_designs': 0, 'single_designs': 0, 'double_designs': 0, 'pocket_overrides': 0, 'sleeve_overrides': 0, 'size_reference_used': 0, 'canvas_scaled': 0, 'original_dimensions_used': 0}
        order_total_counts = {}
        ...
        if not designs:
            ...
            handle_missing_designs_error(missing_orders, file_path, "personalised")
            finish_size_determination_log()
            return

        handle_missing_designs_warning(missing_orders, file_path, "personalised")
        if missing_sizes:
        ...
        finalize_arrangement(gui, batches, show_progress, file_path, "personalised")

    except Exception as e:
        finish_size_determination_log()
        if show_progress:
            reset_progress(gui)
        messagebox.showerror("Error", f"Failed to arrange personalised designs:\n{str(e)}")
        import traceback
        traceback.print_exc()
        error_traceback = ''.join(traceback.format_exception(type(e), e, e.__traceback__))
        from datetime import datetime
        content = f"Failed to Arrange Personalised Designs\n"
        ...
        logger.error("process_personalised_file: exception while processing file=%s error=%s", os.path.basename(file_path) if file_path else None, e)
        log_run_event(
            "processing_failed",
            level="error",
            mode="personalised",
