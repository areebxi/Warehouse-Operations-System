...
| `config/Queue/queue_app_settings.json` | Design folder paths |

| `data/Queue/Configuration Workbook.xlsx` | Pocket / Override Print Size |

| `Custom Label Database/Custom_Label_Database.csv` | Print sizes via universal SKU match |

| `runtime/SharedInbox/DTF Des/` | Auto input |

| `runtime/Queue/{Output,Logs,Missing Size Reference}/` | |
...
**Auto:** watcher on SharedInbox → Missing Logo using settings folders → timestamped PNG under runtime Queue Output → move source to `Processed/` (or `Failed/`). No approval.
